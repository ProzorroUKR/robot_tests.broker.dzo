...
${locator.minimalStep.amount}      xpath=//td[contains(text(),'Мінімальний крок аукціону')]/following-sibling::td/span[1]
${locator.registrationFee.amount}      xpath=//td[contains(text(),'Розмір реєстраційного внеску')]/following-sibling::td/span[1]
...
Отримати інформацію про auctions
  [Arguments]  ${field}  ${username}
    ...
    ...  ELSE IF  'auctionID' in '${field}'  Get Text  xpath=(//a[contains(@href, "/tenders/TenderID")])[${lot_index + 1}]
...
Підтвердити дію
  ${modalok}=   Run Keyword And Return Status  Wait Until Element Is Visible  ${locator.ModalOK}  10
  Run Keyword If  ${modalok}  Wait Until Keyword Succeeds   5 x  1 s  Run Keywords
  ...  Клікнути по елементу   ${locator.ModalOK}
  ...  AND  Wait Until Page Does Not Contain Element  ${locator.ModalOK}
  ...
  ...  AND  Wait Until Page Does Not Contain Element  xpath=//a[./text()= 'Закрити']
...
Скасувати цінову пропозицію
  ...
  Підтвердити дію
  ${status}=   Run Keyword And Return Status   Wait Until Element Is Visible   xpath=//div[@id="contactForm"]/descendant::input[@name="checkMPhone"]  10
  Run Keyword If  ${status}   Run Keywords
  ...  Ввести текст   xpath=//div[@id="contactForm"]/descendant::input[@name="checkMPhone"]    203986723
  ...  AND  Клікнути по елементу   xpath=//button[./text()='Надіслати']
  ...  AND  Wait Until Page Contains Element   xpath=//div[@class="jContent"][contains(text(), "Вашу пропозицію відкликано")]
  Reload Page
...
Завантажити документ в ставку
  ...
  dzo.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
  Wait Until Page Contains Element   xpath=//span[@class="bidAmountValue"]
  ${amount}=  Get Text   xpath=//span[@class="bidAmountValue"]
  dzo.Скасувати цінову пропозицію   ${username}   ${tender_uaid}
  Execute Javascript   $(".bottomFixed").remove();
  Input Text   name=data[value][amount]   ${amount.replace(' ', '')}
...
Відповісти на запитання
  ...
  Wait Until Keyword Succeeds   10 x   1 s  Wait Until Element Is Visible  xpath=//div[contains(text(), '${question_id}')]${locator.questions.answer}
...
Отримати інформацію із запитання
  ...
  ...  Reload Page
  ...  AND  Wait Until Page Contains Element  xpath=//div[contains(text(), '${question_id}')]${locator.questions.${field_name}}
